...
namespace Game
{
    ...
    public class ActionGameWindow : GameWindow
    {
        ...
        private static Vec2 SnapToPixel(Vec2 value, Vec2 viewportSize)
        {
            Vec2 result = value;
            result *= viewportSize;
        ...
        private static Sphere GetSwitchUseAttachedMeshWorldSphere(MapObjectAttachedMesh attachedMesh)
        {
            float radius = 0;
                ...
                string text;
                {
                    ...
                    if (controlItem != null && controlItem.BindedKeyboardMouseValues.Count != 0)
                    {
                        GameControlsManager.SystemKeyboardMouseValue value =
                            controlItem.BindedKeyboardMouseValues[0];
        ...
        private Vec3 CalculateTPSCameraPosition(Vec3 lookAt, Vec3 direction, float maxCameraDistance,
                ...
                case CameraType.FPS:
                    {
                        ...
                        PlayerIntellect.Instance.UpdateTransformBeforeCameraPositionCalculation();
                        unit.GetFirstPersonCameraPosition(out position, out forward, out up); //NEW
                        /*
                                                if (unit is Turret)
                                                {
                                                    //Turret specific
                                                ...
                                                else if (unit is Car)
                                                {
                                                    ...
                                                    up = unit.GetInterpolatedRotation().GetUp(); //for rotational axis
                                                }
                                                else
                                                {
                                                    //Characters, etc
