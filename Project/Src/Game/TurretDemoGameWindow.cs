...
namespace Game
{
    ...
    {
        private float remainingTimeForCreateEnemy;
        private float gameTime;
        private int level;
        private int remainingCount;
        private int remainingCreateCount;
        private float createInterval;

        private Vec3 turretCreatePosition;
        private Quat turretCreateRotation;

        private Font screenFont;

        private List<MapObject> enemySpawnPoints = new List<MapObject>();
        ...
        private void Map_Tick(Entity entity)
        {
            //Game tick
            if (gameTime != 0)
            {
        ...
        private void GameStop(bool complete)
        {
            if (gameTime == 0)
                return;
        ...
        private void UpdateVictoryObjects(bool visible)
        {
            foreach (Entity entity in Map.Instance.Children)
            {
        ...
        private string GetCreateEnemyTypeName()
        {
            float bug = 0;
            float zombie = 0;
            float robot = 0;
        ...
        private void EnemyUnitDestroying(Entity entity)
        {
            if (gameTime != 0)
            {
                remainingCount--;
        ...
        private void MainPlayerUnitSubscribeToDestroying()
        {
        ...
        private void GameBillboard_Damage(Dynamic entity, MapObject prejudicial, Vec3 pos, float damage)
        {
            if (gameTime == 0)
                GameStart();
        }
