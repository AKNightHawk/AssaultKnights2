...
namespace ProjectCommon
{
    ...
    {
        private static EngineConsole instance;

        //visual
        private bool needLoadTextureAndFont = true;

        private Font font;
        private Texture texture;

        private Vec2 textureOffset;
        private float maxTransparency = .9f;

        //commands
        private Command.Method defaultCommandHandler;
        ...
        private bool autoOpening = true;
        private float transparency;

        private struct OldString
        {
            public string text;
        ...
        private List<OldString> strings = new List<OldString>();
        private int stringDownPosition = -1;
        ...
        private void InitInternal()
        {
            ...
            Print("Welcome to the Assault Knights - (NeoAxis Engine)!", new ColorValue(1, 1, 0));
            Print("----------------------------------------------");
        ...
        private void OnConsoleQuit(string arguments)
        {
            EngineApp.Instance.SetNeedExit();
        }

        private void OnConsoleClear(string arguments)
        {
            Clear();
        }

        private void OnConsoleCommands(string arguments)
        {
        ...
        private void OnConsoleFullScreen(string arguments)
        {
            if (!string.IsNullOrEmpty(arguments))
            {
                try
        ...
        private void OnConsoleEngineTimeScale(string arguments)
        {
            if (!string.IsNullOrEmpty(arguments))
            {
                try
        ...
        private void OnConsoleSoundPitchScale(string arguments)
        {
            if (!string.IsNullOrEmpty(arguments))
            {
                try
