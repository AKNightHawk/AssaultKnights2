...
namespace ProjectCommon
{
    ...
    {
        private static bool createEmptyMaterialsForFasterStartupInitialization;

        //General
        private MaterialBlendingTypes blending;

        private bool lighting = true;
        private bool ambientLighting = true;
        private bool doubleSided;
        private bool useNormals = true;
        private bool receiveShadows = true;
        private bool receiveSimpleShadows;
        private CompareFunction alphaRejectFunction = CompareFunction.AlwaysPass;
        private byte alphaRejectValue = 127;
        private bool alphaToCoverage;
        private Range fadingByDistanceRange;
        private bool allowFog = true;
        private bool depthWrite = true;
        private bool depthTest = true;
        private bool softParticles;
        private float softParticlesFadingLength = 1;
        private float depthOffset;
        private bool halfLambert;
        private bool castShadows = true;

        //Diffuse
        private ColorValue diffuseColor = new ColorValue(1, 1, 1);

        private float diffusePower = 1;
        private bool diffuseScaleDynamic;
        private bool diffuseVertexColor;
        private MapItem diffuse1Map;
        private DiffuseMapItem diffuse2Map;
        private DiffuseMapItem diffuse3Map;
        private DiffuseMapItem diffuse4Map;

        private DiffuseMapItem diffuse5Map;
        private DiffuseMapItem diffuse6Map;

        //Reflection
        private ColorValue reflectionColor = new ColorValue(0, 0, 0);

        private float reflectionPower = 1;
        private bool reflectionScaleDynamic;
        private MapItem reflectionMap;
        private string reflectionSpecificCubemap = "";

        //Emission
        private ColorValue emissionColor = new ColorValue(0, 0, 0);

        private float emissionPower = 1;
        private bool emissionScaleDynamic;
        private MapItem emissionMap;

        //Specular
        private ColorValue specularColor = new ColorValue(0, 0, 0);

        private float specularPower = 1;
        private bool specularScaleDynamic;
        private MapItem specularMap;
        private float specularShininess = 20;

        //Translucency
        private ColorValue translucencyColor = new ColorValue(0, 0, 0);

        private float translucencyPower = 1;
        private bool translucencyDynamic = false;
        private float translucencyClearness = 4f;
        private MapItem translucencyMap;

        //Height
        private MapItem normalMap;

        private MapItem heightMap;
        private bool heightFromNormalMapAlpha;
        private DisplacementTechniques displacementTechnique = DisplacementTechniques.ParallaxOcclusionMapping;
        private float heightScale = .04f;

        //Projective texturing
        private bool projectiveTexturing;

        private RenderFrustum projectiveTexturingFrustum;

        //for cubemap reflections
        private List<Pair<Pass, TextureUnitState>> cubemapEventUnitStates;

        //for maps animations
        private List<MapItem> mapsWithAnimations;

        private List<Pass> subscribedPassesForRenderObjectPass;

        private string defaultTechniqueErrorString;
        private bool fixedPipelineInitialized;
        private bool emptyMaterialInitialized;

        private static float mapTransformAnimationTime;
        private static float mapTransformAnimationTimeLastFrameRenderTime;
        ...
        public enum GpuParameters
        {
            ...
            diffuse5MapTransformMul,
            diffuse5MapTransformAdd,
            diffuse6MapTransformMul,
            diffuse6MapTransformAdd,
        ...
        public enum MaterialBlendingTypes
        {
            ...
            AlphaPositive, //Incin
            AlphaFade, //Incin
            SourceColor_Zero,//Incin
            DestColor_Zero,//Incin
            Zero_DestColor,//Incin
        }
        ...
        public class MapItem
        {
            ...
            private TexCoordIndexes texCoord = TexCoordIndexes.TexCoord0;
            private bool clamp;
            private TransformItem transform;
        ...
        public class DiffuseMapItem : MapItem
        {
            ...
            private MapBlendingTypes blending = MapBlendingTypes.Modulate;
        ...
        public class TransformItem
        {
            ...
            private bool dynamicParameters;
        ...
        public class AnimationItem
        {
            ...
            private Vec2 scrollSpeed;
            private Vec2 scrollRound;
        ...
        private bool _useVertexPainting;

        [Category("Diffuse")]
        [LocalizedDisplayName("UseVertexPainting", "ShaderBaseMaterial")]
        [DefaultValue(false)]
        public bool UseVertexPainting
        {
            get { return _useVertexPainting; }
            set { _useVertexPainting = value; }
        }
        ...
        [Category("Diffuse")]
        [LocalizedDisplayName("Diffuse5Map", "ShaderBaseMaterial")]
        public DiffuseMapItem Diffuse5Map
        {
            get { return diffuse5Map; }
            set { diffuse5Map = value; }
        }

        [Category("Diffuse")]
        [LocalizedDisplayName("Diffuse6Map", "ShaderBaseMaterial")]
        public DiffuseMapItem Diffuse6Map
        {
            get { return diffuse6Map; }
            set { diffuse6Map = value; }
        }
        ...
        private MapItem normalMap2;
        private MapItem normalMap3;
        private MapItem normalMap4;

        [Category("Height")]
        [LocalizedDisplayName("NormalMap2", "ShaderBaseMaterial")]
        public MapItem NormalMap2
        {
            get { return normalMap2; }
            set { normalMap2 = value; }
        }

        [Category("Height")]
        [LocalizedDisplayName("NormalMap3", "ShaderBaseMaterial")]
        public MapItem NormalMap3
        {
            get { return normalMap3; }
            set { normalMap3 = value; }
        }

        [Category("Height")]
        [LocalizedDisplayName("NormalMap4", "ShaderBaseMaterial")]
        public MapItem NormalMap4
        {
            get { return normalMap4; }
            set { normalMap4 = value; }
        }

        #endregion NoarmalMap

        #region Specular Map

        private MapItem specularMap2;
        private MapItem specularMap3;
        private MapItem specularMap4;

        [Category("Specular")]
        [LocalizedDisplayName("SpecularMap2", "ShaderBaseMaterial")]
        public MapItem SpecularMap2
        {
            get { return specularMap2; }
            set { specularMap2 = value; }
        }

        [Category("Specular")]
        [LocalizedDisplayName("SpecularMap3", "ShaderBaseMaterial")]
        public MapItem SpecularMap3
        {
            get { return specularMap3; }
            set { specularMap3 = value; }
        }

        [Category("Specular")]
        [LocalizedDisplayName("SpecularMap4", "ShaderBaseMaterial")]
        public MapItem SpecularMap4
        {
            get { return specularMap4; }
            set { specularMap4 = value; }
        }

        #endregion Specular Map

        #region Heightmap

        private MapItem heightMap2;
        private MapItem heightMap3;
        private MapItem heightMap4;

        [Category("Height")]
        [LocalizedDisplayName("HeightMap2", "ShaderBaseMaterial")]
        public MapItem HeightMap2
        {
            get { return heightMap2; }
            set { heightMap2 = value; }
        }

        [Category("Height")]
        [LocalizedDisplayName("HeightMap3", "ShaderBaseMaterial")]
        public MapItem HeightMap3
        {
            get { return heightMap3; }
            set { heightMap3 = value; }
        }

        [Category("Height")]
        [LocalizedDisplayName("HeightMap4", "ShaderBaseMaterial")]
        public MapItem HeightMap4
        {
            get { return heightMap4; }
            set { heightMap4 = value; }
        }
        ...
        private MapItem reflectionMap2;
        private MapItem reflectionMap3;
        private MapItem reflectionMap4;

        [Category("Reflection Cubemap")]
        [LocalizedDisplayName("ReflectionMap2", "ShaderBaseMaterial")]
        public MapItem ReflectionMap2
        {
            get { return reflectionMap2; }
            set { reflectionMap2 = value; }
        }

        [Category("Reflection Cubemap")]
        [LocalizedDisplayName("ReflectionMap3", "ShaderBaseMaterial")]
        public MapItem ReflectionMap3
        {
            get { return reflectionMap3; }
            set { reflectionMap3 = value; }
        }

        [Category("Reflection Cubemap")]
        [LocalizedDisplayName("ReflectionMap4", "ShaderBaseMaterial")]
        public MapItem ReflectionMap4
        {
            get { return reflectionMap4; }
            set { reflectionMap4 = value; }
        }

        #endregion ReflectionMap
        ...
        public ShaderBaseMaterial()
        {
            ...
            diffuse5Map = new DiffuseMapItem(this);
            diffuse6Map = new DiffuseMapItem(this);
            ...
            normalMap2 = new MapItem(this);
            normalMap3 = new MapItem(this);
            normalMap4 = new MapItem(this);

            specularMap2 = new MapItem(this);
            specularMap3 = new MapItem(this);
            specularMap4 = new MapItem(this);

            heightMap2 = new MapItem(this);
            heightMap3 = new MapItem(this);
            heightMap4 = new MapItem(this);

            reflectionMap2 = new MapItem(this);
            reflectionMap3 = new MapItem(this);
            reflectionMap4 = new MapItem(this);
        ...
        protected override void OnClone(HighLevelMaterial sourceMaterial)
        {
            ...
            UseVertexPainting = source.UseVertexPainting;
            ...
            diffuse5Map.OnClone(source.diffuse5Map);
            diffuse6Map.OnClone(source.diffuse6Map);
            ...
            reflectionMap2.OnClone(source.reflectionMap2);
            reflectionMap3.OnClone(source.reflectionMap3);
            reflectionMap4.OnClone(source.reflectionMap4);
            ...
            specularMap2.OnClone(source.specularMap2);
            specularMap3.OnClone(source.specularMap3);
            specularMap4.OnClone(source.specularMap4);
            ...
            normalMap2.OnClone(source.normalMap2);
            normalMap3.OnClone(source.normalMap3);
            normalMap4.OnClone(source.normalMap4);
            ...
            heightMap2.OnClone(source.heightMap2);
            heightMap3.OnClone(source.heightMap3);
            heightMap4.OnClone(source.heightMap4);
        ...
        protected override bool OnLoad(TextBlock block)
        {
            ...
            //Diffuse
            {
                ...
                if (block.IsAttributeExist("useVertexPainting"))
                    _useVertexPainting = bool.Parse(block.GetAttribute("useVertexPainting"));
                ...
                TextBlock diffuse5MapBlock = block.FindChild("diffuse5Map");
                if (diffuse5MapBlock != null)
                    diffuse5Map.Load(diffuse5MapBlock);

                TextBlock diffuse6MapBlock = block.FindChild("diffuse6Map");
                if (diffuse6MapBlock != null)
                    diffuse6Map.Load(diffuse6MapBlock);
            ...
            //Reflection
            {
                ...
                {
                    TextBlock reflectionMapBlock2 = block.FindChild("reflectionMap2");
                    if (reflectionMapBlock2 != null)
                        reflectionMap2.Load(reflectionMapBlock2);
                }
                {
                    TextBlock reflectionMapBlock3 = block.FindChild("reflectionMap3");
                    if (reflectionMapBlock3 != null)
                        reflectionMap3.Load(reflectionMapBlock3);
                }
                {
                    TextBlock reflectionMapBlock4 = block.FindChild("reflectionMap4");
                    if (reflectionMapBlock4 != null)
                        reflectionMap4.Load(reflectionMapBlock4);
                }
            ...
            //Specular
            {
                ...
                {
                    TextBlock specularMapBlock2 = block.FindChild("specularMap2");
                    if (specularMapBlock2 != null)
                        specularMap2.Load(specularMapBlock2);
                }
                {
                    TextBlock specularMapBlock3 = block.FindChild("specularMap3");
                    if (specularMapBlock3 != null)
                        specularMap3.Load(specularMapBlock3);
                }
                {
                    TextBlock specularMapBlock4 = block.FindChild("specularMap4");
                    if (specularMapBlock4 != null)
                        specularMap4.Load(specularMapBlock4);
                }
            ...
            //Height
            {
                ...
                {
                    TextBlock normalMapBlock2 = block.FindChild("normalMap2");
                    if (normalMapBlock2 != null)
                        normalMap2.Load(normalMapBlock2);
                }
                {
                    TextBlock normalMapBlock3 = block.FindChild("normalMap3");
                    if (normalMapBlock3 != null)
                        normalMap3.Load(normalMapBlock3);
                }
                {
                    TextBlock normalMapBlock4 = block.FindChild("normalMap4");
                    if (normalMapBlock4 != null)
                        normalMap4.Load(normalMapBlock4);
                }
                ...
                {
                    TextBlock heightMapBlock2 = block.FindChild("heightMap2");
                    if (heightMapBlock2 != null)
                        heightMap2.Load(heightMapBlock2);
                }
                {
                    TextBlock heightMapBlock3 = block.FindChild("heightMap3");
                    if (heightMapBlock3 != null)
                        heightMap3.Load(heightMapBlock3);
                }
                {
                    TextBlock heightMapBlock4 = block.FindChild("heightMap4");
                    if (heightMapBlock4 != null)
                        heightMap4.Load(heightMapBlock4);
                }
        ...
        protected override void OnSave(TextBlock block)
        {
            ...
            //Diffuse
            {
                ...
                if (_useVertexPainting)
                    block.SetAttribute("useVertexPainting", _useVertexPainting.ToString());
                ...
                if (diffuse5Map.IsDataExists())
                {
                    TextBlock diffuse5MapBlock = block.AddChild("diffuse5Map");
                    diffuse5Map.Save(diffuse5MapBlock);
                }

                if (diffuse6Map.IsDataExists())
                {
                    TextBlock diffuse6MapBlock = block.AddChild("diffuse6Map");
                    diffuse6Map.Save(diffuse6MapBlock);
                }
            }
            ...
            //Reflection
            {
                ...
                {
                    if (reflectionMap2.IsDataExists())
                    {
                        TextBlock reflectionMapBlock2 = block.AddChild("reflectionMap2");
                        reflectionMap2.Save(reflectionMapBlock2);
                    }
                }
                {
                    if (reflectionMap3.IsDataExists())
                    {
                        TextBlock reflectionMapBlock3 = block.AddChild("reflectionMap3");
                        reflectionMap3.Save(reflectionMapBlock3);
                    }
                }
                {
                    if (reflectionMap4.IsDataExists())
                    {
                        TextBlock reflectionMapBlock4 = block.AddChild("reflectionMap4");
                        reflectionMap4.Save(reflectionMapBlock4);
                    }
                }
            ...
            //Specular
            {
                ...
                {
                    if (specularMap2.IsDataExists())
                    {
                        TextBlock specularMapBlock = block.AddChild("specularMap2");
                        specularMap2.Save(specularMapBlock);
                    }
                }
                {
                    if (specularMap3.IsDataExists())
                    {
                        TextBlock specularMapBlock = block.AddChild("specularMap3");
                        specularMap3.Save(specularMapBlock);
                    }
                }
                {
                    if (specularMap4.IsDataExists())
                    {
                        TextBlock specularMapBlock = block.AddChild("specularMap4");
                        specularMap4.Save(specularMapBlock);
                    }
                }
            ...
            //Height
            {
                ...
                {
                    if (normalMap2.IsDataExists())
                    {
                        TextBlock normalMapBlock = block.AddChild("normalMap2");
                        normalMap2.Save(normalMapBlock);
                    }
                }
                {
                    if (normalMap3.IsDataExists())
                    {
                        TextBlock normalMapBlock = block.AddChild("normalMap3");
                        normalMap3.Save(normalMapBlock);
                    }
                }
                {
                    if (normalMap4.IsDataExists())
                    {
                        TextBlock normalMapBlock = block.AddChild("normalMap4");
                        normalMap4.Save(normalMapBlock);
                    }
                }
                ...
                {
                    if (heightMap2.IsDataExists())
                    {
                        TextBlock heightMapBlock = block.AddChild("heightMap2");
                        heightMap2.Save(heightMapBlock);
                    }
                }
                {
                    if (heightMap3.IsDataExists())
                    {
                        TextBlock heightMapBlock = block.AddChild("heightMap3");
                        heightMap3.Save(heightMapBlock);
                    }
                }
                {
                    if (heightMap4.IsDataExists())
                    {
                        TextBlock heightMapBlock = block.AddChild("heightMap4");
                        heightMap4.Save(heightMapBlock);
                    }
                }
        ...
        private void SetProgramAutoConstants_Main_Vertex(GpuProgramParameters parameters, int lightCount)
        {
        ...
        private void SetProgramAutoConstants_Main_Fragment(GpuProgramParameters parameters, int lightCount)
        {
            bool shadowMap = SceneManager.Instance.IsShadowTechniqueShadowmapBased() && ReceiveShadows &&
                lightCount != 0;
        ...
        private void SetProgramAutoConstants_ShadowCaster_Vertex(GpuProgramParameters parameters)
        {
            parameters.SetNamedAutoConstant("worldMatrix",
                GpuProgramParameters.AutoConstantType.WorldMatrix);
            parameters.SetNamedAutoConstant("viewProjMatrix",
        ...
        private void SetProgramAutoConstants_ShadowCaster_Fragment(GpuProgramParameters parameters)
        {
            parameters.SetNamedAutoConstant("farClipDistance",
                GpuProgramParameters.AutoConstantType.FarClipDistance);
        ...
        private void GenerateTexCoordString(StringBuilder builder, int texCoord, TransformItem transformItem,
        ...
        private bool CreateDefaultTechnique(out bool shadersIsNotSupported)
        {
                        ...
                        doubleSidedTwoPassModeCounter++)
                    {
                        ...
                        //configure general pass settings
                        {
                            ...
                            switch (blending)
                            {
                                ...
                                case MaterialBlendingTypes.AlphaSubtract: //Incin
                                    pass.SourceBlendFactor = SceneBlendFactor.Zero;
                                    pass.DestBlendFactor = SceneBlendFactor.SourceColor;
                                    break;

                                case MaterialBlendingTypes.AlphaNegative: //Incin
                                    pass.SourceBlendFactor = SceneBlendFactor.SourceAlpha;
                                    pass.DestBlendFactor = SceneBlendFactor.OneMinusDestColor;
                                    break;

                                case MaterialBlendingTypes.AlphaPositive: //Incin
                                    pass.SourceBlendFactor = SceneBlendFactor.OneMinusSourceAlpha;
                                    pass.DestBlendFactor = SceneBlendFactor.DestColor;
                                    break;

                                case MaterialBlendingTypes.AlphaFade: //Incin
                                    pass.SourceBlendFactor = SceneBlendFactor.SourceColor;
                                    pass.DestBlendFactor = SceneBlendFactor.One;
                                    break;

                                case MaterialBlendingTypes.SourceColor_Zero: //Incin
                                    pass.SourceBlendFactor = SceneBlendFactor.SourceColor;
                                    pass.DestBlendFactor = SceneBlendFactor.Zero;
                                    break;

                                case MaterialBlendingTypes.DestColor_Zero: //Incin
                                    pass.SourceBlendFactor = SceneBlendFactor.DestColor;
                                    pass.DestBlendFactor = SceneBlendFactor.Zero;
                                    break;

                                case MaterialBlendingTypes.Zero_DestColor: //Incin
                                    pass.SourceBlendFactor = SceneBlendFactor.Zero;
                                    pass.DestBlendFactor = SceneBlendFactor.DestColor;
                                    break;
                            }
                        ...
                        int generalTexCoordCount = 4;
                        {
                            ...
                            //Diffuse
                            {
                                ...
                                if (UseVertexPainting)
                                {
                                    generalArguments.Append(" -DUSE_VERTEX_PAINTING");
                                }
                                ...
                                for (int mapIndex = 1; mapIndex <= 6; mapIndex++)
                                {
                                    ...
                                    switch (mapIndex)
                                    {
                                        ...
                                        case 5: map = diffuse5Map; break;
                                        case 6: map = diffuse6Map; break;
                                    }
                                ...
                                (RenderSystem.Instance.IsDirect3D() || RenderSystem.Instance.IsOpenGL()))
                            {
                                    ...
                                    reflectionScaleDynamic)
                                {
                                    ...
                                    for (int mapIndex = 2; mapIndex <= 4; mapIndex++)
                                    {
                                        MapItem map = null;
                                        switch (mapIndex)
                                        {
                                        ...
                                        if (!string.IsNullOrEmpty(map.Texture))
                                        {
                                            generalArguments.AppendFormat(" -DREFLECTION_MAP{0}", mapIndex);
                                            generalArguments.AppendFormat(" -DREFLECTION_MAP{0}_REGISTER=s{1}", mapIndex,
                                                generalSamplerCount);
                                            generalSamplerCount++;
                                ...
                                if (lightPass)
                                {
                                        ...
                                        IsDynamicSpecularScaleAndShininess())
                                    {
                                        ...
                                        for (int mapIndex = 2; mapIndex <= 4; mapIndex++)
                                        {
                                            ...
                                            switch (mapIndex)
                                            {
                                                case 2: map = specularMap2; break;
                                                case 3: map = specularMap3; break;
                                                case 4: map = specularMap4; break;
                                            }

                                            if (!string.IsNullOrEmpty(map.Texture))
                                            {
                                                generalArguments.AppendFormat(" -DSPECULAR_MAP{0}", mapIndex);
                                                generalArguments.AppendFormat(" -DSPECULAR_MAP{0}_REGISTER=s{1}",
                                                    mapIndex, generalSamplerCount);
                                                generalSamplerCount++;
                                ...
                                (RenderSystem.Instance.IsDirect3D() || RenderSystem.Instance.IsOpenGL()))
                            {
                                ...
                                for (int mapIndex = 2; mapIndex <= 4; mapIndex++)
                                {
                                    ...
                                    switch (mapIndex)
                                    {
                                        case 2: map = normalMap2; break;
                                        case 3: map = normalMap3; break;
                                        case 4: map = normalMap4; break;
                                    }

                                    if (!string.IsNullOrEmpty(map.Texture))
                                    {
                                        generalArguments.AppendFormat(" -DNORMAL_MAP{0}", mapIndex);
                                        generalArguments.AppendFormat(" -DNORMAL_MAP{0}_REGISTER=s{1}",
                                            mapIndex, generalSamplerCount);
                                        generalSamplerCount++;
                                    ...
                                    if (!string.IsNullOrEmpty(heightMap.Texture) || HeightFromNormalMapAlpha)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            for (int mapIndex = 2; mapIndex <= 4; mapIndex++)
                                            {
                                                ...
                                                switch (mapIndex)
                                                {
                                                    case 2: map = heightMap2; break;
                                                    case 3: map = heightMap3; break;
                                                    case 4: map = heightMap4; break;
                                                }

                                                if (!string.IsNullOrEmpty(map.Texture))
                                                {
                                                    generalArguments.AppendFormat(" -DHEIGHT_MAP{0}", mapIndex);
                                                    generalArguments.AppendFormat(" -DHEIGHT_MAP{0}_REGISTER=s{1}",
                                                        mapIndex, generalSamplerCount);
                                                    generalSamplerCount++;
        ...
        private void SubscribePassToRenderObjectPassEvent(Pass pass)
        {
        ...
        private void UpdateMapTransformForFixedPipeline(MapItem map)
        {
        ...
        private void FixedPipelineAddDiffuseMapsToPass(Pass pass)
        {
            for (int mapIndex = 1; mapIndex <= 6; mapIndex++)
            {
                MapItem map = null;
        ...
        private void CreateFixedPipelineTechnique()
        {
        ...
        private void CreateEmptyMaterial()
        {
            Technique tecnhique = BaseMaterial.CreateTechnique();
            Pass pass = tecnhique.CreatePass();
        ...
        private void ClearBaseMaterial()
        {
            if (fixedPipelineInitialized)
            {
                ...
                diffuse5Map.textureUnitStatesForFixedPipeline = null;
                diffuse6Map.textureUnitStatesForFixedPipeline = null;
        ...
        private void InitializeAndUpdateDynamicGpuParameters()
        {
            ...
            InitializeAndUpdateMapTransformGpuParameters(diffuse5Map);
            InitializeAndUpdateMapTransformGpuParameters(diffuse6Map);
        ...
        private void SetCustomGpuParameter(GpuParameters parameter, Vec4 value, bool vertex, bool fragment,
        ...
        private bool IsDynamicDiffuseScale()
        {
        ...
        private void UpdateDynamicDiffuseScaleGpuParameter()
        {
            if (IsDynamicDiffuseScale())
            {
                ColorValue scale = DiffuseColor *
                    new ColorValue(DiffusePower, DiffusePower, DiffusePower, 1);
                if (CastStaticShadows == true)
                    SetCustomGpuParameter(GpuParameters.dynamicDiffuseScale, scale.ToVec4(), false, true, true);
                else
                    SetCustomGpuParameter(GpuParameters.dynamicDiffuseScale, scale.ToVec4(), false, true, false);
            }
        }

        private bool IsDynamicEmissionScale()
        {
        ...
        private void UpdateDynamicEmissionScaleGpuParameter()
        {
            if (IsDynamicEmissionScale())
            {
        ...
        private bool IsDynamicReflectionScale()
        {
        ...
        private void UpdateDynamicReflectionScaleGpuParameter()
        {
            if (IsDynamicReflectionScale())
            {
        ...
        private bool IsDynamicSpecularScaleAndShininess()
        {
            return specularScaleDynamic || specularColor != new ColorValue(0, 0, 0);
        }

        private void UpdateDynamicSpecularScaleAndShininessGpuParameter()
        {
            if (IsDynamicSpecularScaleAndShininess())
            {
                ColorValue scale = specularColor * specularPower;
        ...
        private bool IsDynamicTranslucencyScaleAndClearness()
        {
            return translucencyDynamic || translucencyColor != new ColorValue(0, 0, 0);
        }

        private void UpdateDynamicTranslucencyScaleAndClearnessGpuParameter()
        {
            if (IsDynamicTranslucencyScaleAndClearness())
            {
                ColorValue scale = translucencyColor * translucencyPower;
        ...
        private void UpdateFadingByDistanceRangeGpuParameter()
        {
        ...
        private void UpdateSoftParticlesFadingLengthGpuParameter()
        {
            if (softParticles)
            {
        ...
        private void InitializeAndUpdateMapTransformGpuParameters(MapItem map)
        {
        ...
        private static float GetMapTransformAnimationTime()
        {
        ...
        private void UpdateMapTransformGpuParameters(MapItem map)
        {
            TransformItem transform = map.Transform;
            ...
            bool needForShadowCasterMaterial = false;
            {
                ...
                else if (map == diffuse5Map)
                {
                    mulGpuParameter = GpuParameters.diffuse5MapTransformMul;
                    addGpuParameter = GpuParameters.diffuse5MapTransformAdd;
                    needForShadowCasterMaterial = true;
                }
                else if (map == diffuse6Map)
                {
                    mulGpuParameter = GpuParameters.diffuse6MapTransformMul;
                    addGpuParameter = GpuParameters.diffuse6MapTransformAdd;
                    needForShadowCasterMaterial = true;
                }
        ...
        private void Pass_RenderObjectPass(Pass pass, Vec3 objectWorldPosition)
        {
        ...
        private void UpdateReflectionCubemap(TextureUnitState textureUnitState, Vec3 objectWorldPosition)
        {
            string textureName = "";
        ...
        private void PreloadTexture(string textureName, Texture.Type textureType)
        {
            if (!string.IsNullOrEmpty(textureName))
            {
        ...
        protected override void OnPreloadResources()
        {
            ...
            PreloadTexture(diffuse5Map.Texture, Texture.Type.Type2D);
            PreloadTexture(diffuse6Map.Texture, Texture.Type.Type2D);
            ...
            PreloadTexture(normalMap2.Texture, Texture.Type.Type2D);
            PreloadTexture(normalMap3.Texture, Texture.Type.Type2D);
            PreloadTexture(normalMap4.Texture, Texture.Type.Type2D);

            PreloadTexture(specularMap2.Texture, Texture.Type.Type2D);
            PreloadTexture(specularMap3.Texture, Texture.Type.Type2D);
            PreloadTexture(specularMap4.Texture, Texture.Type.Type2D);

            PreloadTexture(heightMap2.Texture, Texture.Type.Type2D);
            PreloadTexture(heightMap3.Texture, Texture.Type.Type2D);
            PreloadTexture(heightMap4.Texture, Texture.Type.Type2D);

            PreloadTexture(reflectionMap2.Texture, Texture.Type.Type2D);
            PreloadTexture(reflectionMap3.Texture, Texture.Type.Type2D);
            PreloadTexture(reflectionMap4.Texture, Texture.Type.Type2D);
        ...
        {
            return new MapItem[] { diffuse1Map, diffuse2Map, diffuse3Map, diffuse4Map, diffuse5Map, diffuse6Map, reflectionMap, emissionMap, specularMap,
