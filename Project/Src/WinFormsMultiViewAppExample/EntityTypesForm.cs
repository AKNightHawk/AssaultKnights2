...
namespace WinFormsMultiViewAppExample
{
    public partial class EntityTypesForm : WeifenLuo.WinFormsUI.Docking.DockContent
    {
        ...
        private void NodeCreate(EntityType type)
        {
        ...
        private TreeNode GetChildGroupNodeByName(TreeNodeCollection groupNodeChildren, string name)
        {
        ...
        private TreeNode GroupNodeFindByPath(string path)
        {
        ...
        private TreeNode GroupNodeCreate(string path)
        {
            string[] listPath = path.Split("\\/".ToCharArray());
            Trace.Assert(listPath.Length != 0, "listPath.Length != 0");
        ...
        private void GroupsDeleteOneChild()
        {
            while (true)
            {
