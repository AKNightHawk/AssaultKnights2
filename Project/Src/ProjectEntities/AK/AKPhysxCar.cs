...
{
    /// <summary>
    /// Defines the <see cref="AKCar"/> entity type.
    /// </summary>
    public class CarType : AKunitType
    {
        //Masses
        [FieldSerialize]
        private float massChassis = 1500;

        //Differential
        [FieldSerialize]
        private PhysicsVehicle.DifferentialTypes differentialType = PhysicsVehicle.DifferentialTypes.LimitedSlipAllWheelDrive;

        [FieldSerialize]
        private float differentialFrontRearSplit = 0.45f;

        [FieldSerialize]
        private float differentialFrontLeftRightSplit = 0.5f;

        [FieldSerialize]
        private float differentialRearLeftRightSplit = 0.5f;

        [FieldSerialize]
        private float differentialCenterBias = 1.3f;

        [FieldSerialize]
        private float differentialFrontBias = 1.3f;

        [FieldSerialize]
        private float differentialRearBias = 1.3f;

        //Engine
        [FieldSerialize]
        private float enginePeakTorque = 500;

        [FieldSerialize]
        private float engineMaxRPM = 6000;

        [FieldSerialize]
        private float engineDampingRateFullThrottle = .15f;

        [FieldSerialize]
        private float engineDampingRateZeroThrottleClutchEngaged = 2.0f;

        [FieldSerialize]
        private float engineDampingRateZeroThrottleClutchDisengaged = .35f;

        [FieldSerialize]
        private List<EngineTorqueCurveItem> engineTorqueCurve = new List<EngineTorqueCurveItem>();

        //Gears
        [FieldSerialize]
        private List<GearItem> gears = new List<GearItem>();

        [FieldSerialize]
        private float gearsSwitchTime = .5f;

        //Clutch
        [FieldSerialize]
        private float clutchStrength = 10;

        //Ackermann steer accuracy
        [FieldSerialize]
        private float ackermannSteerAccuracy = 1;

        //Wheels
        [FieldSerialize]
        private WheelItem wheelFrontLeft = new WheelItem(WheelNames.FrontLeft);

        [FieldSerialize]
        private WheelItem wheelFrontRight = new WheelItem(WheelNames.FrontRight);

        [FieldSerialize]
        private WheelItem wheelRearLeft = new WheelItem(WheelNames.RearLeft);

        [FieldSerialize]
        private WheelItem wheelRearRight = new WheelItem(WheelNames.RearRight);

        //Tire types
        [FieldSerialize]
        private List<TireTypeItem> tireTypes = new List<TireTypeItem>();

        //steer vs forward speed table
        [FieldSerialize]
        private List<SteerVsForwardSpeedTableItem> steerVsForwardSpeedTable = new List<SteerVsForwardSpeedTableItem>();
        ...
        {
            [FieldSerialize]
            private TireTypeNames name = TireTypeNames.Wets;

            //Wheels
            [FieldSerialize]
            private TireWheelItem wheelFrontLeft = new TireWheelItem(WheelNames.FrontLeft);

            [FieldSerialize]
            private TireWheelItem wheelFrontRight = new TireWheelItem(WheelNames.FrontRight);

            [FieldSerialize]
            private TireWheelItem wheelRearLeft = new TireWheelItem(WheelNames.RearLeft);

            [FieldSerialize]
            private TireWheelItem wheelRearRight = new TireWheelItem(WheelNames.RearRight);

            //Material friction multipliers
            [FieldSerialize]
            private List<FrictionMultiplierItem> frictionMultipliers = new List<FrictionMultiplierItem>();
            ...
            {
                private WheelNames wheelName;

                [FieldSerialize]
                private float tireLatStiffX = 2.0f;

                [FieldSerialize]
                private float tireLatStiffY = 18;//0.3125f * ( 180.0f / 3.14159f );

                [FieldSerialize]
                private float tireLongitudinalStiffnessPerUnitGravity = 1000.0f;

                [FieldSerialize]
                private float tireCamberStiffness = 57;//1.0f * ( 180.0f / 3.14159f );

                [FieldSerialize]
                private float frictionVsSlipGraphZeroLongitudinalSlip = 1.0f;

                [FieldSerialize]
                private float frictionVsSlipGraphLongitudinalSlipWithMaximumFriction = 0.1f;

                [FieldSerialize]
                private float frictionVsSlipGraphMaximumFriction = 1.0f;

                [FieldSerialize]
                private float frictionVsSlipGraphEndPointOfGraph = 1.0f;

                [FieldSerialize]
                private float frictionVsSlipGraphValueOfFrictionForSlipsGreaterThanEndPointOfGraph = 1.0f;
        ...
        public class WheelItem
        {
            ...
            [FieldSerialize]
            private float wheelDampingRate = 0.25f;

            [FieldSerialize]
            private float wheelMaxBrakeTorque = 1500.0f;

            [FieldSerialize]
            private float wheelMaxHandBrakeTorque;

            [FieldSerialize]
            private Degree wheelMaxSteer;

            [FieldSerialize]
            private Degree wheelToeAngle;

            [FieldSerialize]
            private float suspensionSpringStrength = 35000;

            [FieldSerialize]
            private float suspensionSpringDamperRate = 4500;

            [FieldSerialize]
            private float suspensionMaxCompression = .3f;

            [FieldSerialize]
            private float suspensionMaxDroop = .1f;

            [FieldSerialize]
            private float suspensionSprungMassCoefficient = .25f;

            [FieldSerialize]
            private Vec3 suspensionForceApplicationPointOffset = new Vec3(0, 0, 0);

            [FieldSerialize]
            private Vec3 tireForceApplicationPointOffset = new Vec3(0, 0, 0);
    ...
    public class Car : AKunit, ProfilingToolWindow.ICarInfo
    {
        [FieldSerialize]
        private CarType.TireTypeNames tireType = CarType.TireTypeNames.Wets;

        [FieldSerialize]
        private bool autoGear = true;

        [FieldSerialize]
        private bool autoGearAutoReverse = true;
        ...
        private MapObjectAttachedObject[] wheelAttachedObjects;

        //networking support. data on client machine to transfer position of wheels.
        private Radian[] networkingClient_wheelsRotationAngle;

        private Radian[] networkingClient_wheelsSteer;
        private float[] networkingClient_wheelsSuspensionJounce;
        ...
        //        if (!OnGround)
        //        {
        //            if (Intellect.IsControlKeyPressed(GameControlKeys.ArrowUp))
        //            {
        ...
        private void TickGearsAutoReverse()
        {
            if (!IsInAir())
            {
        ...
        protected override void OnTick()
        {
            ...
            if (physicsVehicle != null)
            {
                ...
                physicsVehicle.BaseBody.AddForce(ForceType.Global, TickDelta, new Vec3(0, 0, accel *= 9.87f/*Type.PhysicsGravity*/), Position);
        ...
        private void UpdateWheelSettings(PhysicsVehicle.InitDataClass.WheelItem destination, CarType.WheelItem source,
        ...
        private void CreatePhysicsVehicle()
        {
        ...
        private void DestroyPhysicsVehicle()
        {
            if (physicsVehicle != null)
            {
        ...
        private void Server_SendWheelsPositionToAllClients()
        {
        ...
        private void Client_ReceiveWheelsPositionToClient(RemoteEntityWorld sender, ReceiveDataReader reader)
        {
