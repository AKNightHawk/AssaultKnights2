...
namespace ProjectEntities
{
    ...
    public class PlayerIntellect : Intellect
    {
        ...
        private static PlayerIntellect instance;

        [FieldSerialize]
        private SphereDir lookDirection;

        private bool fpsCamera;//for hiding player unit for the fps camera
        private float tpsCameraCenterOffset;

        //data for an opportunity of the player to control other units. (for Example: Turret control)
        [FieldSerialize]
        private Unit mainNotActiveUnit;

        private Dictionary<Shape, int> mainNotActiveUnitShapeContactGroups;

        [FieldSerialize]
        private Vec3 mainNotActiveUnitRestorePosition;

        private Vec3 client_sentTurnToPosition = new Vec3(float.NaN, 0, 0);
        ...
        private PlayerIntellectType _type = null; public new PlayerIntellectType Type { get { return _type; } }
        ...
        private bool GetTurnToPosition(out Vec3 turnToPosition)
        {
            if (Instance != this)
        ...
        private void UpdateMainControlledUnitAfterLoading()
        {
            if (mainNotActiveUnit == null)
                return;
        ...
        private void AlternativeUnitAllowPlayerControl_Destroying(Entity entity)
        {
        ...
        private Vec3 FindFreePositionForUnit(Unit unit, Vec3 center)
        {
        ...
        private void TickCurrentUnitAllowPlayerControl()
        {
            if (mainNotActiveUnit == null)
                return;
        ...
        private void Client_ReceiveSetInstance(RemoteEntityWorld sender, ReceiveDataReader reader)
        {
        ...
        private void Server_ReceiveChangeMainControlledUnit(RemoteEntityWorld sender,
        ...
        private void Server_ReceiveRestoreMainControlledUnit(RemoteEntityWorld sender,
            ReceiveDataReader reader)
        {
            //check to ensure that other players can not send messages to another player
            if (!Server_CheckRemoteEntityWorldAssociatedWithThisIntellect(sender))
        ...
        private void Client_SendTurnToPositionToServer(Vec3 turnToPosition)
        {
            float epsilon = .01f;
        ...
        private void Server_ReceiveTurnToPosition(RemoteEntityWorld sender, ReceiveDataReader reader)
        {
            //check to ensure that other players can not send messages to another player
            if (!Server_CheckRemoteEntityWorldAssociatedWithThisIntellect(sender))
                return;
        ...
        private void Server_SendMainNotActiveUnitToClients(IList<RemoteEntityWorld> remoteEntityWorlds)
        {
        ...
        private void Client_SendControlKeyReleaseToServer(GameControlKeys controlKey)
        {
            SendDataWriter writer = BeginNetworkMessage(typeof(PlayerIntellect),
        ...
        public bool Server_CheckRemoteEntityWorldAssociatedWithThisIntellect(RemoteEntityWorld sender)
        {
        ...
        private void Server_ReceiveControlKeyPress(RemoteEntityWorld sender, ReceiveDataReader reader)
        {
            //check to ensure that other players can not send messages to another player
            if (!Server_CheckRemoteEntityWorldAssociatedWithThisIntellect(sender))
                return;
        ...
        private void Server_ReceiveControlKeyRelease(RemoteEntityWorld sender, ReceiveDataReader reader)
        {
            //check to ensure that other players can not send messages to another player
            if (!Server_CheckRemoteEntityWorldAssociatedWithThisIntellect(sender))
                return;
