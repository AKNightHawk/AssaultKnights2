...
namespace ProjectEntities
{
    ...
    public class GameCharacterAI : AI
    {
        ...
        [FieldSerialize]
        private AutomaticTasksEnum automaticTasks = AutomaticTasksEnum.Enabled;

        [FieldSerialize]
        private float findAutomaticTaskRemainingTime;

        private Task currentTask;
        private Queue<Task> tasks = new Queue<Task>();
        private ReadOnlyQueue<Task> tasksReadOnly;

        //#region Incin
        private PathController pathController = new PathController();

        [FieldSerialize]
        private bool alwaysRun; //= true;  //Incin(bin) == true otherwise false;

        //[FieldSerialize]
        //private bool alwaysAttack = true;  //Incin(bin) == true otherwise false;

        //[FieldSerialize]
        //private bool alwaysHeal = true;  //Incin(bin) == true otherwise false;

        //[FieldSerialize]
        //private bool alwaysControl = true;  //Incin(bin) == true otherwise false;

        //[FieldSerialize]
        //private bool alwaysAI = true;  //Incin(bin) == true otherwise false;
        ...
        {
            private GameCharacterAI owner;

            private Vec3 taskPosition;
            private MapObject taskEntity;
            ...
            internal bool _Load(TextBlock block)
            {
                return OnLoad(block);
            }
            ...
            internal void _Save(TextBlock block)
            {
                OnSave(block);
            }
        ...
        private class PathController
        {
            ...
            private readonly int maxPolygonPath = 512;
            private readonly int maxSmoothPath = 4096;
            private readonly int maxSteerPoints = 16;

            private Vec3 foundPathForTargetPosition = new Vec3(float.NaN, float.NaN, float.NaN);
            private Vec3[] path;
        ...
        private GameCharacterAIType _type = null; public new GameCharacterAIType Type { get { return _type; } }
        ...
        private Task CreateTaskByClassName(string className)
        {
        ...
        private static Vec3[] addSpherePositions;
        private static int[] addSphereIndices;

        private static void AddSphere(Camera camera, Sphere sphere)
        {
        ...
        private static Vec3[] addThicknessLinePositions;
        private static int[] addThicknessLineIndices;

        private static void AddThicknessLine(Camera camera, Vec3 start, Vec3 end, float thickness)
        {
            Vec3 diff = end - start;
        ...
        private static void AddCone(Camera camera, Vec3 from, Vec3 to, float radius)
        {
        ...
        private static void AddArrow(Camera camera, Vec3 from, Vec3 to, float thickness, float arrowSize)
        {
