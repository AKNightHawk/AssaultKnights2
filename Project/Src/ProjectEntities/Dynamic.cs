...
namespace ProjectEntities
{
    ...
    public class DynamicType : MapObjectType
    {
        ...
        private static float shieldmax = 1000;

        [FieldSerialize]
        private float shieldMax = shieldmax;

        [FieldSerialize]
        private float shieldMin;

        [FieldSerialize]
        private float impulseDamageCoefficient;

        [FieldSerialize]
        private float impulseMinimalDamage;

        [FieldSerialize]
        private float targetPriority;

        [FieldSerialize]
        private string soundCollision;

        [FieldSerialize]
        private float soundCollisionMinVelocity = 1.0f;

        [FieldSerialize]
        private Substance substance;

        [FieldSerialize]
        private float dieLatency;

        [FieldSerialize]
        private float lifeTime;

        [FieldSerialize]
        private List<AutomaticInfluenceItem> automaticInfluences = new List<AutomaticInfluenceItem>();
        ...
        [FieldSerialize]
        private SoundRolloffModes soundRolloffMode = SoundRolloffModes.Logarithmic;

        [FieldSerialize]
        private float soundMinDistance = 5;

        [FieldSerialize]
        private float soundMaxDistance = 50;

        [FieldSerialize]
        private float soundRolloffLogarithmicFactor = 1;

        private MapObjectCreateObjectCollection dieObjects = new MapObjectCreateObjectCollection();
        ...
        {
            [FieldSerialize]
            private InfluenceType influence;

            [FieldSerialize]
            [DefaultValue(typeof(Range), "0 0.5")]
            private Range lifeCoefficientRange = new Range(0, .5f);
        ...
        protected override bool OnLoad(TextBlock block)
        {
            ...
            //old version compatibility
            if (block.IsAttributeExist("Max"))
                shieldMax = float.Parse(block.GetAttribute("shieldMax"));
            if (block.IsAttributeExist("lifeMin"))
                shieldMin = float.Parse(block.GetAttribute("shieldMin"));
    ...
    public class Dynamic : MapObject
    {
        ...
        private float healthFactorAtBeginning = 1;

        [FieldSerialize(FieldSerializeSerializationTypes.World)]
        private float health;

        private static float shielddefualt = 1000;

        [FieldSerialize(FieldSerializeSerializationTypes.World)]
        private float shield = shielddefualt;

        [FieldSerialize]
        [DefaultValue(0.0f)]
        private float dieLatencyRamainingTime;

        private bool died;

        private float soundCollisionTimeRemaining;
        private bool soundCollisionTimeRemainingTimerAdded;
        ...
        [FieldSerialize]
        [DefaultValue(1.0f)]
        private float receiveDamageCoefficient = 1;

        [FieldSerialize]
        private Influence[] automaticInfluences;

        private bool server_enableSynchronizationPositionsToClients = true;

        //for NetworkMessages.PositionToClient (PhysicsModel == null)
        private Vec3 server_sentPositionToClients = new Vec3(float.MaxValue, 0, 0);

        private Quat server_sentRotationToClients = Quat.Identity;
        private Vec3 server_sentScaleToClients = new Vec3(1, 1, 1);

        //for NetworkMessages.BodiesPositionsToClient (PhysicsModel != null)
        private Server_SentBodiesPositionsToClientsItem[] server_sentBodiesPositionsToClients;

        //we are using cache of positions for interpolation
        private List<Client_ReceivePositionsSnapshot> client_receivePositionsSnapshots = new List<Client_ReceivePositionsSnapshot>();

        //Die objects
        private bool dieObjectsCreated;

        ///////////////////////////////////////////

        //for NetworkMessages.BodiesPositionsToClient (PhysicsModel != null)
        private struct Server_SentBodiesPositionsToClientsItem
        {
        ...
        private class Client_ReceivePositionsSnapshot
        {
        ...
        private DynamicType _type = null; public new DynamicType Type { get { return _type; } }
        ...
        public float Shield
        {
            get
            {
                //MathFunctions.Clamp(ref shield, Type.ShieldMin, Type.ShieldMax);
                return shield;
            }
            set
            {
                ...
                if (shield < Type.ShieldMin)
                    shield = Type.ShieldMin;
                else if (shield > Type.ShieldMax)
                    shield = Type.ShieldMax;
                ...
                if (EntitySystemWorld.Instance.IsServer() &&
                    Type.NetworkType == EntityNetworkTypes.Synchronized)
                {
                    Server_SendShieldToClients(EntitySystemWorld.Instance.RemoteEntityWorlds);
                }
            }
        }
            ...
            bool allowMoveDamageToParent)
        {
            ...
            if (Type.HealthMax != 0)
            {
                float newHealth = Health - (realDamage * .25f);

                if (Type.ShieldMax != 0 && Shield > 0f)
                {
                    float newShield = Shield - (realDamage * .75f);
                    if (newShield < Type.ShieldMin)
                    {
                        newHealth = Health - realDamage;
                    }
                    MathFunctions.Clamp(ref newShield, Type.ShieldMin, Type.ShieldMax);
                    //newShield = Shield - (.75f * realDamage);
                    Shield = newShield;
                }
                else
                {
        ...
        protected override void OnTick()
        {
            ...
            {
                if (health > 0 && health < Type.HealthMax)
                {
                    HealthRegen();
                    ShieldRegen();
                }
        ...
        private void CopyInfluencesToObject(Dynamic destination)
        {
            foreach (Entity child in Children)
            {
                if (child.IsSetForDeletion)
        ...
        private void UpdateAutomaticInfluences()
        {
            for (int n = 0; n < Type.AutomaticInfluences.Count; n++)
            {
                DynamicType.AutomaticInfluenceItem typeItem = Type.AutomaticInfluences[n];
            ...
            RemoteEntityWorld remoteEntityWorld)
        {
            ...
            if (Health != Type.HealthMax)
            {
                ...
                Server_SendShieldToClients(new RemoteEntityWorld[] { remoteEntityWorld });
            }
        }
        ...
        private void Server_SendPositionsToAllClients(bool updateAll)
        {
        ...
        private void Server_SendPositionsToNewClient(RemoteEntityWorld remoteEntityWorld)
        {
        ...
        private void Client_ReceivePositions(RemoteEntityWorld sender, ReceiveDataReader reader)
        {
        ...
        private void Server_SendBodiesPositionsToNewClient(RemoteEntityWorld remoteEntityWorld)
        {
            SendDataWriter writer = BeginNetworkMessage(remoteEntityWorld, typeof(Dynamic),
                (ushort)NetworkMessages.BodiesPositionsToClient);
        ...
        private void Client_ReceiveBodiesPositions(RemoteEntityWorld sender, ReceiveDataReader reader)
        {
        ...
        private void Server_SendHealthToClients(IList<RemoteEntityWorld> remoteEntityWorlds)
        {
        ...
        private void Client_ReceiveHealth(RemoteEntityWorld sender, ReceiveDataReader reader)
        {
        ...
        private void Server_SendShieldToClients(IList<RemoteEntityWorld> remoteEntityWorlds)
        {
            SendDataWriter writer = BeginNetworkMessage(remoteEntityWorlds, typeof(Dynamic),
                (ushort)NetworkMessages.ShieldToClient);
            writer.Write(Health);
            EndNetworkMessage();
        }

        [NetworkReceive(NetworkDirections.ToClient, (ushort)NetworkMessages.ShieldToClient)]
        ...
        private void Server_SendSoundPlayCollisionToAllClients()
        {
            SendDataWriter writer = BeginNetworkMessage(typeof(Dynamic),
        ...
        private void Client_ReceiveSoundPlayCollision(RemoteEntityWorld sender, ReceiveDataReader reader)
        {
            if (!reader.Complete())
                return;
            SoundPlay3D(Type.SoundCollision, .5f, false);
        ...
        private void Client_GetReceivePositionsSnapshots(int networkTickNumber,
        ...
        private void Server_SendDieCallToAllClients(MapObject prejudicial, bool allowLatencyTime)
        {
        ...
        private void Client_ReceiveDieCall(RemoteEntityWorld sender, ReceiveDataReader reader)
        {
        ...
        {
            if (health > 0 && health < Type.HealthMax)
                health += .45f * (health / Type.HealthMax);
            //else
            //    return 0f;
        }
        ...
        {
            if (shield >= 0 && shield < Type.ShieldMax)
            {
                if (shield <= 0)
                    shield = 1f;

                if (shield < Type.ShieldMax / 10f)
                {
                    shield += 10f;
                }
                else if (shield < Type.ShieldMax / 5)
                {
                    shield += .80f * (shield / Type.ShieldMax);
                }
                else if (shield < Type.ShieldMax / 3)
                {
                    shield += .70f * (shield / Type.ShieldMax);
                }
                else
                {
                    shield += .65f * (shield / Type.ShieldMax);
                }
